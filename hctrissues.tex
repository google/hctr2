...
even when \(X = \lambda\),
so as a special case \(H(\lambda) = \hgen\).

Unfortunately, as \cite{kumarhctr} observes this is no longer
an injective map from \(X\)---we also have \(H(0) = \hgen\). This
...
\(Y_i^r \qeq \UU^s\) occurs iff \(\hgen\) is one of the zeroes
...
infer for \(S_i^r\) depends on \(\hgen\). The values of \(\hgen\)
