...
class PolyHash(Hash):
    ...
    def poly(self, m, key):
        ...
        blocks = [m[i:i + self.lengths()['blocksize']]
                  for i in range(0, len(m), self.lengths()['blocksize'])]
        hash_result = self.gf(0)
        for i in range(len(blocks)):
            exponent = (len(blocks) - 1) - i
            hash_result += (hmont**exponent) * \
                self.gf(int.from_bytes(blocks[i], byteorder='little'))
    ...
    def hash(self, key, message, tweak):
        ...
        if(len(message) % self.lengths()['blocksize'] == 0):
            blocks += (len(tweak) * 8 * 2 + 2).to_bytes(self.lengths()
                                                        ['blocksize'], byteorder='little')
            blocks += self.pad(tweak)
            blocks += message
            blocks += b'\x00' * self.lengths()['blocksize']
        else:
            blocks += (len(tweak) * 8 * 2 + 3).to_bytes(self.lengths()
                                                        ['blocksize'], byteorder='little')
            blocks += self.pad(tweak)
