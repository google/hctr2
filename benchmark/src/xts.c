...
void xts_setkey(struct aes_xts_ctx *ctx, const u8 *key)
{
    ...
    asmlinkage void aesni_xts_encrypt(const struct crypto_aes_ctx *ctx, u8 *dst,
            const u8 *src, unsigned int len, const le128 *iv);
    ...
    asmlinkage void ce_aes_xts_encrypt(u8 out[], u8 const in[], u8 const rk1[], int rounds,
            int bytes, u8 const rk2[], const u8 iv[], int first);
...
#ifdef __x86_64__
    le128 alpha;
    aesni_ecb_enc(&ctx->tweak_ctx.aes_ctx, (u8*)&alpha, iv, XTS_BLOCK_SIZE);
    aesni_xts_encrypt(&ctx->crypt_ctx.aes_ctx, dst, src, nbytes, &alpha);
#endif
#ifdef __aarch64__
    ce_aes_xts_encrypt(dst, src, (u8*)&ctx->tweak_ctx.aes_ctx.key_enc, 14, nbytes, (u8*)&ctx->crypt_ctx.aes_ctx.key_enc, iv, true);
#endif
}
...
#endif
#ifdef __aarch64__
    ce_aes_xts_decrypt(dst, src, (u8*)&ctx->tweak_ctx.aes_ctx.key_dec, 14, nbytes, (u8*)&ctx->crypt_ctx.aes_ctx.key_enc, iv, true);
#endif
}
