...
\begin{itemize}
    ...
    For query \(i\), this is 
    \begin{displaymath}
        B^i = \XCTR_\pi(S^i)[\abs{P^i}; nm^i-\abs{P^i}]
    \end{displaymath}
...
(\(0^n\), \(10^{n-1}\), \(\MM^i\), \(S_j^i\))
or any pair of inferred block cipher ciphertexts are the same
(\(h\), \(L\), \(\VV^i\), \(Y_j^i\)). 

FIXME: the below is false as stated: need to introduce
a notion of ``compatibility''. Simpler to consider
compatibility with attacker than with world.

Since responses in world Y are coin flips,
the probability of a particular transcript, good or bad,
in world Y is always simply \(2^-{\mtot n}\).
For a ``good'' transcript, the probability in world X
