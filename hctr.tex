...
\begin{align*}
    ...
    Y_j &= \pi(S_j) \\
\end{align*}
We therefore infer \(\sigma_m = 2 + \sum_i m^i \) block cipher
...
\begin{itemize}
    \item \(\UU^s \qeq \UU^{s'}\): Given both queries and responses,
    this holds if \(U^s \xor H_h(T^s, C^s) = U^{s'} \xor H_h(T^{s'}, C^{s'})\);
    by hash function property 2, this holds with probability
    \(\max(d(T^s, C^s), d(T^{s'}, C^{s'}))/2^n\).
    \item \(Y_j^s \qeq \UU^{s'}\): Given both queries and responses,
    this holds if \(Y_j^s = U^{s'} \xor H_h(T^{s'}, C^{s'})\);
    by hash function property 1, this holds with probability
    \(\max(d(T^s, C^s), d(T^{s'}, C^{s'}))/2^n\).
    \item \(UU^s \qeq Y_j^{s'}\): Given \(h\), all possible
