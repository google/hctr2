...
Where \(|M| \equiv 0 \pmod n\) we define polynomial evaluation as \(\Poly_h(\lambda) = 0^n\), \(\Poly_h(M \Concat T) = h \odot (\Poly_h(M) \xor T)\) where \(|T| = n\). This is insensitive to prefixed zeroes \(\Poly_h(0^n \concat M) = \Poly_h(M)\) and linear in the second argument \(\Poly_h(M) \xor \Poly_h(M') = \Poly_h(M \xor M')\).
...
\begin{figure}
	\begin{floatrow}
        ...
        \ffigbox{
            ...
            \caption{Pseudocode for Modified CTR}\label{ctrpseudocode}
        }
	\end{floatrow}
\end{figure}
