...
and report on its implementation on x86 and arm64.
...
We interpret \(n\)-bit blocks as field elements of \(\GF(2^n)\)
...
For tweak \(T\) and  message \(M\) we define a polynomial
\begin{displaymath}
    H(T, M) = \poly(\fromint_{n}(\abs{T} + 2) \Concat \pad_n(T) \Concat \pad_n(M) \Concat 0^n)
\end{displaymath}
...
Tweaks may be any length from 0 to \(2^n-2\) bits, while
messages may be any length \(n\) or greater.
...
The first term of \(H(T, M)\) is always nonzero,
