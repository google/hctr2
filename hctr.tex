...
\begin{itemize}
    ...
    \item HCTR uses a fixed-length tweak. To make standardization
    easier, HCTR2 handles a variable-length tweak.
    \item HCTR's key is a block cipher key, plus an \(n\)-bit
    hash key. To simplify deployment, HCTR2's key is simply
...
\begin{align*}
    ...
    &+ \left.\left(3\sigma^2 + 2q\sigma + 10\sigma + q^2 + 2\right)\middle/2^{n+1}\right. \\
\end{align*}
