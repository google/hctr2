...
evaluate a polynomial at the parameter \(h\),
but at another point, \(\hmont = x^{-n}h\).
...
\defeq  M_0\hmont^{l-1} \xor \cdots \xor M_{l-1}\).
While \(\hmont + 2\) and \(2\hmont + 1\) can be equal
in value if \(\hmont = 1\), two formal polynomials are only equal
if every coefficient is equal; thus \(\poly(M) = \poly(M')\)
only if \(M = 0^{ln} \Concat M'\) for some \(l\) or vice versa.

We then define \(H(T, M)\) as the formal polynomial in \(\hmont\) given
by that tweak and message.
\(H_h(T, M)\) is then evaluation of this polynomial at
\(\hmont = x^{-n}h\).
\begin{align*}
    & H(T, M) \\
    \defeq &
...
\begin{itemize}
    ...
    \item The polynomial is never \(0\) or \(x^n\hmont\)
...
in which case the polynomial is \(x\hmont\).
Since \(x^{n-1} \neq 1\) we have that \(x^n \neq x\).

For any nonzero polynomial \(p(\hmont)\)
in \(GF(2^n)\), there are at most \(\deg(p)\) values \(\hmont\)
such that \(p(\hmont) = 0\), and therefore
\(\probsub{\hmont\sample{\bin^n}}{p(\hmont) = 0} \leq \deg(p)/2^n\).
Since multiplication by a nonzero field element
is a bijection of the field onto itself, it follows that
\(\probsub{h\sample{\bin^n}}{p(x^{-n}h) = 0} \leq \deg(p)/2^n\).
...
\begin{description}
    ...
    cannot be equal to the polynomial \(x^n\hmont\).
    Substituting \(h = x^n\hmont\),
    \(H(T, M) \xor g \xor h = H(T, M) \xor g \xor x^n\hmont\)
