...
\begin{align*}
    ...
    \intertext{where}
    c \defeq &\sum_{\mathcal{M}\in{\mathcal{D}, \mathcal{R}}}
    \sum_{a, b \in \mathcal{M}} 2^n\prob{a = b} -1
\end{align*}
This way of counting has the advantage that
the fourteen grey boxes in \autoref{domaincollision} and \autoref{rangecollision}
require a correction of zero and so needn't be considered further;
only the two red and six green boxes need to be considered.
We proceed by cases:
...
\begin{align*}
    ...
    \leq & \left.\left(2\binom{\sigma_m}{2} - 1 + 2(\sigma - q) + (q-1)\sigma + \binom{\sigma}{2}\right)\middle/2^n\right. \\
