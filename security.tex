...
For efficiency reasons, POLYVAL does not
evaluate a polynomial at the parameter \(\hgen\),
but at another point, \(\hpoly = x^{-n}\hgen\).
Some care is therefore needed to relate
properties of the hash function to
possible zeroes of a polynomial.
...
While \(\hpoly + 2\) and \(2\hpoly + 1\) can be equal
in value if \(\hpoly = 1\), two formal polynomials are only equal
...
will collide, we find the sum of
probability of collision for all pairs in 
the multiset \(\mathcal{D}\)
and all pairs in the multiset \(\mathcal{R}\). 
Since most pairs have a probability of colliding of \(1/2^n\),
we define a ``correction'' \(c\) such that 
\begin{align*}
    & \prob{Y \in \Tb} \\
    = &\prob{\exists a, b \in \mathcal{D}: a = b \vee \exists a, b \in \mathcal{R} : a = b} \\
    \leq &\sum_{\mathcal{M}\in{\mathcal{D}, \mathcal{R}}}
        \sum_{a, b \in \mathcal{M}} \prob{a = b} \\
    = &\left.\left(2\binom{\sigma_m}{2} + 
    \sum_{\mathcal{M}\in{\mathcal{D}, \mathcal{R}}}
    \sum_{a, b \in \mathcal{M}} 2^n\prob{a = b} -1
    \right)\middle/2^n\right. \\
    = &\left.\left(2\binom{\sigma_m}{2} + c
    \right)\middle/2^n\right.
\end{align*}
We proceed by cases:
\(c = c_f + c_w + c_a + c_b\) where
\begin{itemize}
    \item \(c_f\) considers collisions
    for all \(s\)
    from \(\{\fromint(0), \fromint(1)\}\) to \(\{\MM^s, S_1^s, \ldots, S_{m^s-1}^s\}\)
    and from \(\{\hgen, L\}\) to \(\{\UU^s, Y_1^s, \ldots, Y_{m^s-1}^s\}\)
    \item \(c_w\) considers collisions
    for all \(s\)
    within \(\{\MM^s, S_1^s, \ldots, S_{m^s-1}^s\}\)
    and within \(\{\UU^s, Y_1^s, \ldots, Y_{m^s-1}^s\}\)
    \item \(c_a\) considers collisions
    for all \(r < s\)
    from \(\{\MM^r, S_1^r, \ldots, S_{m^r-1}^r\}\) to \(\{\MM^s, S_1^s, \ldots, S_{m^s-1}^s\}\)
    and from \(\{\UU^r, Y_1^r, \ldots, Y_{m^r-1}^r\}\) to \(\{\UU^s, Y_1^s, \ldots, Y_{m^s-1}^s\}\)
    \item \(c_b\) considers collisions within 
    \(\{\fromint(0), \fromint(1)\}\) and \(\{\hgen, L\}\)
\end{itemize}
With this formulation, the grey boxes in
\autoref{domaincollision} and \autoref{rangecollision}
require a correction of zero and so needn't be considered further;
only the red and green boxes need to be considered.
...
\begin{align*}
    ...
    \leq & \prob{Y \in \Tb} \\
    \leq & \left.\left(2\binom{\sigma_m}{2} + c_f + c_w + c_a + c_b\right)\middle/2^n\right. \\
    \leq & \left.\left(2\binom{\sigma_m}{2} + 2(\sigma - q) + (q-1)\sigma + \binom{\sigma}{2} -1\right)\middle/2^n\right. \\
